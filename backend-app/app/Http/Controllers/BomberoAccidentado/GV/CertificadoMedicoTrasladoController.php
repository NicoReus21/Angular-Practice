...
class CertificadoMedicoTrasladoController extends DocumentController
{
    public function store(Request $request, Process $process)
    {
        ...
        $document = $this->upload($process, $file, 'otro', 'certificado_medico_traslado', $request->user()->id);

        if ($document) {
            return response()->json([
                'success' => true,
                'message' => 'Certificado Médico de Traslado subido correctamente',
                'document' => $document,
            ], 201);
        }
