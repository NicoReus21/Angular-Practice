...
class BoletaMedicamentoController extends DocumentController
{
    public function store(Request $request, Process $process)
    {
        ...
        $document = $this->upload($process, $file, 'prestacion_medica', 'boleta_medicamentos', $request->user()->id);

        if ($document) {
            return response()->json([
                'success' => true,
                'message' => 'Boleta de Medicamento subida correctamente',
                'document' => $document,
            ], 201);
        }
