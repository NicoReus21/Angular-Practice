...
export class DocumentUploadComponent implements OnInit {
  ...
  isCurrentSectionCompleted = computed(() => {
    ...
    return currentSection.steps.filter((step) => !step.optional).every((step) => step.isCompleted);
  });
  ...
  setActiveStep(sectionIndex: number, stepIndex: number): void {
    const targetFlatIndex = this.getFlatIndex(sectionIndex, stepIndex);
    this.flatStepIndex.set(targetFlatIndex);
  }
  ...
  private getSectionKeyForStepTitle(stepTitle: string): string {
    ...
    if (title.startsWith('orden de atención médica'))
      return 'certificado_medico_atencion_especial_id';
    if (title.startsWith('informe médico')) return 'informe_medico_id';
    if (title.startsWith('certificado superintendente'))
      return 'certificado_acreditacion_voluntario_id';
    if (title.startsWith('incidente sin lesiones copia del libro de guardia'))
      return 'copia_libro_guardia_id';
    ...
    if (title.startsWith('boleta o factura de gastos de traslado'))
      return 'boleta_factura_traslado_id';
    if (title.startsWith('certificado del médico tratante que determine la incapacidad'))
      return 'certificado_medico_incapacidad_id';
    if (title.startsWith('certificado médico tratante que justifique la necesidad de traslado'))
      return 'certificado_medico_traslado_id';
    if (title.startsWith('boleta de gastos de hospedaje')) return 'boleta_gasto_acompanante_id';
    if (title.startsWith('boleta de gastos de alimentación'))
      return 'boleta_alimentacion_acompanante_id';
    ...
    this.uploadService.getProcessById(id).subscribe({
      next: (record) => {
        this.preliminaryForm.patchValue({
          bomberoNombre: record.bombero_name,
        ...
        const firstIncompleteIndex = allSteps.findIndex(
          (step) => !step.isCompleted && !step.optional
        );

        this.flatStepIndex.set(
          firstIncompleteIndex === -1 ? allSteps.length - 1 : firstIncompleteIndex
        );
  ...
  submitPreliminaryData() {
    ...
    this.uploadService
      .createProcess(this.preliminaryForm.value)
      ...
      .subscribe({
        ...
        error: (err) => {
          this.uploadError.set('Error al crear el proceso.');
        },
      });
  }
  ...
  public nextStep(): void {
    ...
    if (currentStep.optional || currentStep.files.length > 0) {
      ...
      this.snackBar.open('Debes subir un archivo para continuar.', 'Cerrar', { duration: 3000 });
    }
  }
  ...
  handleFiles(fileList: FileList | null, step: UploadStep): void {
    ...
    this.uploadService
      .uploadFile(processId, step.title, file)
      .pipe(
        finalize(() => {
      ...
      .subscribe({
        next: (response) => {
          if (response && response.document) {
            ...
            if (this.flatStepIndex() < this.allSteps().length - 1) {
              this.advanceToNextStep();
            }
          } else {
    ...
    if (this.isServerFile(fileToRemove)) {
      this.uploadService.deleteFile(fileToRemove.id).subscribe({
        next: () => {
          ...
          if (!step.optional && step.files.length === 0) {
            step.isCompleted = false;
            const stepIndex = this.allSteps().indexOf(step);
            if (stepIndex !== -1) {
              this.flatStepIndex.set(stepIndex);
            }
          }
        ...
        error: (err) => {
          this.uploadError.set(`Error al eliminar el archivo "${fileToRemove.name}" del servidor.`);
        },
      });
    } else {
      step.files = step.files.filter((f) => f !== fileToRemove);
  ...
  saveDraft(): void {
    this.snackBar.open('Borrador guardado (archivos subidos correctamente).', 'Cerrar', {
      duration: 3000,
    });
  ...
  finishProcess(): void {
    ...
    if (incompleteMandatoryStep) {
      this.snackBar.open(`Falta completar: ${incompleteMandatoryStep.title}`, 'Cerrar', {
        duration: 5000,
      });
      const index = allSteps.indexOf(incompleteMandatoryStep);
      this.flatStepIndex.set(index);
      return;
    }
    ...
    return [
      {
        ...
        steps: [
          ...
          {
            title: 'DIAB (declaracion Individual de accidente bomberil)',
            optional: false,
            files: [],
            isCompleted: false,
          },
          ...
          {
            title: 'Declaracion de testigos (si es que aplica)',
            optional: true,
            files: [],
            isCompleted: false,
          },
          {
            title:
              'Incidente sin lesiones Copia del Libro de Guardia (no legalizado y solo el registro del accidente)',
            optional: true,
            files: [],
            isCompleted: false,
          },
      ...
      },
      {
        ...
        steps: [
          ...
          {
            title: 'Informe Médico emitido por médico tratante, con diagnóstico y prestaciones',
            optional: false,
            files: [],
            isCompleted: false,
          },
          {
            title: 'Otros Documentos de caracter Médico adicional',
            optional: true,
            files: [],
            isCompleted: false,
          },
      ...
      },
      {
        ...
        steps: [
          {
            title: 'Certificado Superintendente que acredite calidad voluntario',
            optional: false,
            files: [],
            isCompleted: false,
          },
          {
            title:
              'Copia libro de llamadas (central de Alarmas) referido a 3 días previos y posteriores al accidente autorizado ante notario.',
            optional: true,
            files: [],
            isCompleted: false,
          },
          {
            title: 'Copia Aviso de citación al acto de servicio (ACADEMIA).',
            optional: false,
            files: [],
            isCompleted: false,
          },
          {
            title:
              'Copia Lista de Asistencia al acto de servicio específico (ACADEMIA), autorizado ante notario.',
            optional: false,
            files: [],
            isCompleted: false,
          },
          {
            title:
              'Informe Ejecutivo sobre el acto de servicio en que se producen lesiones, suscrito por Superintendente y Comandante.',
            optional: false,
            files: [],
            isCompleted: false,
          },
      ...
      },
      {
        ...
        steps: [
          {
            title: 'Factura establecimiento hospitalario, con detalle de prestaciones.',
            optional: false,
            files: [],
            isCompleted: false,
            isPaid: false,
          ...
          {
            title:
              'Boletas de Honorarios Profesionales, no incluidas en factura, visadas por medico jefe del servicio.',
            optional: true,
            files: [],
            isCompleted: false,
            isPaid: false,
          ...
          {
            title: 'Boleta de Medicamentos y prescripción correspondiente.',
            optional: false,
            files: [],
            isCompleted: false,
            isPaid: false,
          ...
          {
            title:
              'Certificado del director del servicio que autoriza exámenes, recetas, medicamentos, controles, traslados, acciones médicas y procedimientos generales...',
            optional: true,
            files: [],
            isCompleted: false,
          },
      ...
      },
      {
        ...
        steps: [
          {
            title: 'Boleta o factura de gastos de traslado del voluntario',
            optional: true,
            files: [],
            isCompleted: false,
          },
          {
            title:
              'Certificado del médico tratante que determine la incapacidad de asistir al voluntario...',
            optional: true,
            files: [],
            isCompleted: false,
          },
          {
            title:
              'Certificado médico tratante que justifique la necesidad de traslado del voluntario y del medio empleado.',
            optional: true,
            files: [],
            isCompleted: false,
          },
          {
            title: 'Boleta de gastos de hospedaje del acompañante del voluntario.',
            optional: true,
            files: [],
            isCompleted: false,
          },
          {
            title: 'Boleta de gastos de alimentación del acompañante del voluntario.',
            optional: true,
            files: [],
            isCompleted: false,
          },
